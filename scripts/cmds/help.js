...
module.exports = {
  config: {
    ...
    role: 0,
    shortDescription: {
      en: "عرض الأوامر حسب الأقسام",
      ar: "عرض الأوامر حسب الأقسام"
    },
    longDescription: {
      en: "عرض الأوامر مقسمة حسب التصنيف مع إمكانية عرض أوامر قسم معين",
      ar: "عرض الأوامر مقسمة حسب التصنيف مع إمكانية عرض أوامر قسم معين"
    },
    category: "النظام",
    guide: {
      en: "{pn} [رقم القسم أو اسم الأمر]",
  ...
  onStart: async function ({ message, args, event, threadsData, role }) {
    ...
    if (args.length > 0 && isNaN(parseInt(args[0]))) {
      ...
      const longDescription = configCommand.longDescription?.ar || configCommand.longDescription?.en || "لا يوجد وصف.";
      ...
      } else if (typeof configCommand.guide?.en === "string") {
        usage = configCommand.guide.en
          .replace(/{p}/g, prefix)
          .replace(/{n}/g, configCommand.name);
      }
...
╰━━━━━━━━━━━━━❖`;
    ...
    commandsInCategory.forEach(({ name, cmd }, index) => {
      const desc = cmd.config.shortDescription?.ar || cmd.config.shortDescription?.en || "بدون وصف.";
      msg += `${index + 1}. ${prefix}${name}\n» ${desc}\n\n`;
    });

    msg += `\n💡 عدد الأوامر: ${commandsInCategory.length}\n`;
    msg += `🧠 اكتب "${prefix}مساعدة [اسم الأمر]" لرؤية تفاصيل أمر محدد.`;
